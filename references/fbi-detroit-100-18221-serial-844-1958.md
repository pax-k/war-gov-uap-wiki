---
title: "FBI Detroit Office Memorandum — Weaver Sighting (17 Apr 1958, Serial 844)"
category: references
tags: [uap, primary-source, fbi, 1958, declassified]
sources: [sources/65_hs1-101634279_100-de-18221_serial_844.json]
summary: One-page declassified FBI Detroit Office Memorandum (SA Reynolds → SAC Detroit, 17 Apr 1958), serial 844 in file 100-18221, recording a 0408-hrs civilian phone-in by David Weaver of a circular crystal-domed object over Detroit.
provenance:
  extracted: 0.7
  inferred: 0.28
  ambiguous: 0.02
base_confidence: 0.7
lifecycle: draft
lifecycle_changed: 2026-05-09
created: 2026-05-09T15:25:29Z
updated: 2026-05-09T15:25:29Z
---

# FBI Detroit Office Memorandum — Weaver Sighting (17 Apr 1958, Serial 844)

A **one-page Standard Form No. 64 "Office Memorandum, UNITED STATES GOVERNMENT"** (carbon, declassification authority "FBI Automatic Declassification Guide, Issued May 24, 2007") from **Special Agent Robert Ross Reynolds** to **SAC, Detroit**, dated **17 April 1958**, subject "**UNIDENTIFIED FLYING OBJECTS**". The artifact is filed as **serial 844 of FBI file 100-18221** (Detroit field office), with a cross-reference to file `100-26505`. Screened by NARA (RF) on 11-25-2025 under FOIA #90288, document ID `34715588`.

It is the **earliest FBI-originated UAP-adjacent record** in the wiki by ~65 years — predating the [[entities/fbi|FBI's]] [[references/fbi-fd302-drone-pilot-redacted-2023|2023 FD-302 pair]] on the Sept 2023 contractor-convoy incident, and the corpus's **first 1950s** artifact of any kind. ^[inferred]

## Document anatomy

| Field | Value |
|---|---|
| Form | Standard Form No. 64, "Office Memorandum • UNITED STATES GOVERNMENT" |
| To | SAC, Detroit |
| From | SA Robert Ross Reynolds |
| Date | April 17, 1958 |
| Subject | UNIDENTIFIED FLYING OBJECTS |
| Initials / drafting block | `RRR/mac` (1) |
| Field office | FBI Detroit |
| File number | `100-18221`, serial **844** (footer: `100-18221-844`) |
| Cross-file | `100-26505` |
| Stamp | "FOIA/PA — DO NOT DESTROY SERIAL — PAGE 10 — 844 — 8/89" |
| Receipt stamp | "SEARCHED ___ INDEXED ___ SERIALIZED ___ FILED ___ APR 17 1958 — FBI - DETROIT" |
| Endorsement | "Lt. Col (Bermnie) E. Prater advised 4/17/58, no letter rec. mps" ^[ambiguous] |

The endorsement spelling "Bermnie" is almost certainly an OCR artifact for "**Bernie**" (or similar). ^[inferred]

## What the memo records

A **citizen phone-in to the FBI Detroit field office** at **0408 hrs, 17 April 1958**, by:

- **David Weaver**, age 23, of **15801 Decosta, Detroit, Michigan**, telephone **KEnwood 6-7546**.
- Identified himself as the son of a Detroit policeman, with prior Civil Air Patrol (CAP) experience but "not too familiar with the identification of aircraft."
- Stated he was returning home from his Detroit employment when he made the observation.

**The observation as reported by Weaver:**

- **Object**: described as **"a circular object with a crystal-type dome that reflected lights."**
- **Heading**: passing "in a northern direction from southwest."
- **Track**: "crossed the City three blocks south of Six Mile at Lamphere Street."
- **Witness's first action**: tried to telephone **Selfridge Field** (the AF base NE of Detroit) to reach the Air Force; could not get through.
- **Second action**: telephoned the FBI Detroit field office instead.

**Disposition by FBI Detroit:**

- Reynolds memo's **Recommendation** section reads in full: "Advise proper air force authorities."
- The endorsement records that "**Lt. Col (Bermnie) E. Prater advised 4/17/58, no letter rec. mps**" — i.e., the Air Force POC was telephonically advised the same day; no follow-up letter received.

## Provenance and routing

The memo's footer file marker `100-18221-844` shows that **at least 843 prior serials** existed in FBI Detroit file `100-18221` by 17 April 1958, and that this was filed as item 844 in the same case file. The cross-reference to `100-26505` indicates a **second related file** in Detroit's `100`-series caseload (the `100-` prefix is FBI's standard "Domestic Security" classification). ^[inferred] Whether file `100-18221` is itself a UFO-subject case or a broader security file under which UFO phone-ins were sub-filed is **unresolved** by this single serial. ^[inferred]

The receipt stamp uses Detroit's standard intake apparatus (`SEARCHED / INDEXED / SERIALIZED / FILED`) — the same routing skeleton seen on later FBI documents. The 1958 routing is structurally similar to the later [[entities/fbi|FBI]] FD-302 / case-file workflow: a **citizen contact → SA memo → SAC routing → external POC notification**. The form factor differs — a Standard Form 64 office memorandum is a *routing instrument*, not a *witness-interview evidentiary form* like the FD-302; consistent with the report being **the SA's secondhand summary of a phone call**, not a sit-down interview. ^[inferred]

## Position in the corpus

This artifact occupies a **previously empty cell** in the wiki's UAP timeline:

| Era | Existing corpus anchors | This artifact |
|---|---|---|
| 1944–45 | [[references/foo-fighters-shaef-1944-1945]], [[references/sighting-cylindrical-object-vicinity-f5710-1945-03-01]] | — |
| 1948 | [[references/usaf-flying-discs-1948]], [[references/sighting-bakersfield-1948-03]], [[references/sighting-hobson-ohio-1948-05]] | — |
| 1949–50 | [[references/usaf-flying-discs-1949]] (~20 sightings) | — |
| **1958** | *(empty before this artifact)* | **THIS** |
| 1963 | [[references/hunter-1963-space-alien-race-memo]] (NASC policy memo) | — |
| 1985, 1994 | [[references/dos-cable-papua-new-guinea-1985-01]], [[references/dos-cable-kazakhstan-1994-01]] | — |
| 2023 | [[references/fbi-fd302-drone-pilot-redacted-2023]], [[references/fbi-fd302-serial-3-driver-redacted-2023]] (FBI FD-302 pair) | — |
| 2025 | [[references/usper-statement-2025]] | — |
| 2026 | [[references/western-us-event-2026]] | — |

It is the **wiki's first 1950s** artifact, and the earliest FBI-originated UAP-adjacent record in the corpus. It also predates the 1963 [[references/hunter-1963-space-alien-race-memo|NASC policy memo]] by 5 years and falls **between** the 1948–50 USAF Project SIGN era and the 1963 NASC policy-staff layer — a window that historically corresponds to **Project BLUE BOOK** (1952–1969). ^[inferred]

## What this serial *does* and *does not* establish

**It establishes:**

- That as of April 1958, the FBI Detroit field office was an **available citizen-contact channel for UFO sightings**, and that Detroit's intake protocol on receiving such a call was: **(a)** record an SA memo to the SAC, **(b)** advise the local Air Force POC by phone, and **(c)** serialize and file in the case folder.
- That the citizen, **David Weaver**, attempted to reach the **Air Force first** (via Selfridge Field) and only contacted the FBI when that channel failed — i.e., the public still recognised the **Air Force** as the lead UAP-reporting authority, with the FBI as fallback. ^[inferred] Consistent with [[concepts/flying-disc-reporting-protocol|Cabell's 1948 directive]] having ten years to propagate into citizen awareness.
- That the FBI's 1958 **administrative response** to a UFO call was *not* to investigate; it was to **route the report to the Air Force**. The FBI **did not record the kind of sighting detail** (10-element protocol, sketched track, multi-witness corroboration) that a Project SIGN / BLUE BOOK intake would produce.

**It does not establish:**

- Whether Selfridge / "Lt. Col Prater" actually opened a Project BLUE BOOK file on the Weaver sighting. ^[open]
- Whether file `100-18221` is **subject-named** "UFO" or whether UFO matters were sub-filed under a broader Detroit security case. ^[inferred]
- Anything about the object's actual nature; the report contains **one descriptive line** ("circular object with a crystal-type dome that reflected lights") and one trajectory line. The phenomenology is too thin to support morphology classification beyond *circular + dome* — though the descriptor is **on-thesis** for the **classic 1950s "saucer-with-dome" morphology**. ^[inferred]

## Cross-references

- **First FBI-originated UAP-adjacent record** in the corpus → see [[entities/fbi]] (entity has been broadened to span 1958 → 2023).
- **Sighting page** for the Weaver observation: [[references/sighting-detroit-1958-04-17]].
- **Selfridge Field** as a recurring citizen-recognised AF point of contact for UFO calls — track recurrence in future ingests. ^[open]
- **Sister institutional layer**: this 1958 memo is to the late-Project-SIGN / early-BLUE-BOOK era what the [[references/dos-cable-papua-new-guinea-1985-01|1985 PNG cable]] is to the diplomatic-channel layer — a **routing instrument**, not an investigation report. ^[inferred]

## Open threads

- Locate / ingest **other serials in FBI Detroit file 100-18221** — the marker `100-18221-844` implies at least 843 prior items in the same file; only this single serial is in the corpus. ^[open]
- Locate the cross-referenced file `100-26505` (Detroit). ^[open]
- Confirm "**Lt. Col (Bermnie) E. Prater**" — likely a Selfridge Field officer; OCR'd as "Bermnie", probably "**Bernie**". ^[ambiguous]
- Cross-check the **Project BLUE BOOK** master list for an April 1958 Detroit / Selfridge entry matching the Weaver morphology and 0408 LT track. ^[open]
- Look for a corresponding **USAF / Selfridge** memo or BLUE BOOK case file referencing Weaver, the Lamphere Street track, or the 0408-LT phone call. ^[open]
- Identify whether the cross-file `100-26505` has its own UFO context, or whether it's a **person-of-interest** file that intersects this serial only incidentally. ^[inferred]

## See also

- [[entities/fbi]] — intake authority (1958 → 2023 span)
- [[references/sighting-detroit-1958-04-17]] — the consolidated sighting narrative
- [[concepts/flying-disc-reporting-protocol]] — the prior USAF intake protocol whose citizen-recognition this artifact corroborates
- [[references/fbi-fd302-drone-pilot-redacted-2023]] — modern FBI UAP-adjacent record (artifact-level), for instrument comparison
- [[projects/uap/uap]]
