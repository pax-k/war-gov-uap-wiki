...
sources: [sources/usper-statement-redacted.json, sources/38_143685_box7_incident_summaries_173-233.json, sources/38_143685_box7_incident_summaries_101-172.json, sources/38_143685_box7_incident_summaries_1-100.json, sources/65_hs1-834228961_62-hq-83894_serial_130.json, sources/dow-uap-d4-mission-report-arabian-gulf-2020.json, sources/dow-uap-d5-mission-report-arabian-gulf-2020.json, sources/dow-uap-d7-mission-report-arabian-gulf-2020.json, sources/dow-uap-d54-mission-report-mediterranean-sea-na.json]
...
The four substantive mission-report ingests from the [[entities/dow-uap-foia-release|DoW-UAP FOIA release]] — [[references/dow-uap-d4-mission-arabian-gulf-2020|DoW-UAP-D4]] (single sighting at 1258Z, 321 kt, speed-up + eastward turn), [[references/dow-uap-d5-mission-arabian-gulf-2020|DoW-UAP-D5]] (two sightings: 1354Z 40-kt constant-velocity single-object at FL160-170, and 2243Z 278-kt two-object speed-up + southward turn), [[references/dow-uap-d7-mission-arabian-gulf-2020|DoW-UAP-D7]] (single sighting, balloon morphology, wind-borne drift at 31,000 ft MSL, weapons-quality track + NEXT TO SHOOT + TFLIR ID), and [[references/dow-uap-d54-mission-mediterranean-sea|DoW-UAP-D54]] (single sighting at 1319Z during RTB, triangular-and-metallic morphology, witness aircraft at 24,989 ft MSL / 168 KT over the Aegean Sea, no UAP-side kinematics) — are **kinematics-and-morphology mission-report observations** that do **not** record UAP-side engagement-class behavior. Across N=4 reports / 5 datums, there is **no close approach**, **no UAP co-location with the aircraft for non-trivial time**, **no target-selection across aircraft**, **no phase-of-flight correlation**. None of the five datums meet the 4-criterion UAP-toward-aircraft engagement-class bar above. ^[inferred]

This is **expected and useful** as a negative datum at N=4: the modern dow-uap operational-reporting framework produces **brief-observation-class records** that document the UAP-side signature without the close-proximity / multi-aircraft-selection envelope that anchors the engagement class. Engagement-class events should remain a small minority of the modern corpus, distinguished from the broader brief-observation report stream. ^[inferred] If a future `dow-uap-` `d*` ingest documents close approach + cross-aircraft target selection by a UAP, *that* artifact would promote to engagement-class evidence and supersede this negative datum.

**Four sub-classes within brief-observation now emerge from `d4` + `d5` + `d7` + `d54`** (5 datums, all `^[inferred]`):
...
4. **Morphology-rich kinematics-thin sub-class** (`d54` sole) — **NEW at N=4.** Triangular-and-metallic UAP morphology over the Aegean Sea / Cyclades, during the witness aircraft's RTB at 168 KT / 24,989 ft MSL. **Inverts the d4/d5/d7 pattern**: witness-aircraft kinematic state (position + altitude + speed) is fully quantified; **UAP-side kinematics are not reported** (no UAP altitude, no UAP velocity, no UAP motion descriptor). The signature is *what the UAP looked like* rather than *how it moved*. Unlike the d7 prosaic-candidate (balloon), the triangular metallic morphology has **no obvious conventional candidate** in the Eastern-Mediterranean civilian fleet — broadly consistent with the classic "black triangle" UAP class in civilian-research literature, though no further morphology details (size, lighting, wingspan) are preserved. ^[inferred]
